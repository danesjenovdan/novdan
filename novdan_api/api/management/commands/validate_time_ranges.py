    ...
    def handle(self, *args, **options):
        ...
        for subscription in Subscription.objects.all():
            ...
            for time_range in subscription.time_ranges.all():
                self.stdout.write(f'SubscriptionTimeRange: user={subscription.user} id={time_range.id}')

                if time_range.starts_at.month != time_range.ends_at.month or time_range.starts_at.year != time_range.ends_at.year:
                ...
                year_month_string = f'{time_range.starts_at.year}-{time_range.starts_at.month}'
        ...
        if len(problematic_time_ranges):
            raise CommandError(f'Problems with {len(problematic_time_ranges)} time range(s)!')
